...
void SBattleTanksButtonWidget::Construct(const FArguments& InArgs)
{
	...
	ChildSlot
		...
		[
			SNew(SImage)
			.Image(this, &SBattleTanksButtonWidget::GetButtonImage)
			.ColorAndOpacity(this, &SBattleTanksButtonWidget::GetImageColor)
		]
	+ SOverlay::Slot()
		.HAlign(HAlign_Center)
		.VAlign(VAlign_Center)
		[
			SNew(SImage)
			.Image(this, &SBattleTanksButtonWidget::GetButtonImage)
		.ColorAndOpacity(this, &SBattleTanksButtonWidget::GetTintColor)
		]
	+ SOverlay::Slot()
		...
		.Padding(TAttribute<FMargin>(this, &SBattleTanksButtonWidget::GetTextMargin))
		[
			SNew(STextBlock)
			.ShadowColorAndOpacity(this, &SBattleTanksButtonWidget::GetTextShadowColor)
		.ColorAndOpacity(this, &SBattleTanksButtonWidget::GetTextColor)
		.ShadowOffset(FIntPoint(-1, 1))
		.Font(this, &SBattleTanksButtonWidget::GetTextFont)
		.Text(ButtonText)
		]
	+ SOverlay::Slot()
...
float SBattleTanksButtonWidget::GetCurrentOpacity() const
{
	return (Opacity.Get().IsSet() ? Opacity.Get().GetValue() : 1.0f) * FMath::Lerp(0.0f, 1.0f, OpacityCurve.GetLerp());
}

FSlateColor	SBattleTanksButtonWidget::GetImageColor() const
{
	return FLinearColor(1, 1, 1, GetCurrentOpacity());
}

FSlateColor	SBattleTanksButtonWidget::GetCoinColor() const
{
	return FLinearColor(1, 1, 1, GetCurrentOpacity());
}

FSlateColor	SBattleTanksButtonWidget::GetTextColor() const
{
	const FLinearColor GoldColor = FLinearColor(FColor(255, 213, 160, GetCurrentOpacity() * 255));
	return GoldColor;
}

FLinearColor SBattleTanksButtonWidget::GetTextShadowColor() const
{
	return FLinearColor(0, 0, 0, GetCurrentOpacity());
}

EVisibility SBattleTanksButtonWidget::GetCoinVisibility() const
{
	return CoinIconVisible.Get().IsSet() ? CoinIconVisible.Get().GetValue() : EVisibility::Collapsed;
}

FMargin	SBattleTanksButtonWidget::GetTextMargin() const
{
...
const FSlateBrush*  SBattleTanksButtonWidget::GetButtonImage() const
{
	...
	{
		return FMenuStyles::Get().GetBrush("DefaultActionImageBrush");
	}
}

void SBattleTanksButtonWidget::SetImage(UTexture2D* Texture)
{
	if (Texture != NULL)
	{
		ButtonImage.Reset();
		...
		AlphaMap.Reset();
		AlphaMap = FBattleTanksHelper::CreateAlphaMapFromTexture(Texture);
	}
}

void SBattleTanksButtonWidget::SetActionAllowed(bool bEnabled)
{
	bIsActionAllowed = bEnabled;
}

void SBattleTanksButtonWidget::SetActionActive(bool bIsActive)
{
	bIsActiveAction = bIsActive;
}

void SBattleTanksButtonWidget::SetUserActionRequired(bool bIsWaitingForUser)
{
	bIsUserActionRequired = bIsWaitingForUser;
}


FSlateColor SBattleTanksButtonWidget::GetTintColor() const
{
	...
	if (bIsUserActionRequired)
	{
		...
		{
			ResultTint.A = (1.0f + FMath::Sin(MainMenuHUD->GetWorld()->GetTimeSeconds()*4.0f)) / 4.0f;
		}
	}
	return  FLinearColor(
...
FReply SBattleTanksButtonWidget::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	bIsMouseButtonDown = true;
	return FReply::Handled();
}

FReply SBattleTanksButtonWidget::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SBattleTanksButtonWidget::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	OnMouseEnterDel.ExecuteIfBound(MyGeometry, MouseEvent);
}

void SBattleTanksButtonWidget::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	bIsMouseButtonDown = false;
...
FCursorReply SBattleTanksButtonWidget::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	return FCursorReply::Cursor(EMouseCursor::Default);
}

FReply SBattleTanksButtonWidget::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (bIsMouseButtonDown)
	{
		bIsMouseButtonDown = false;
...
void SBattleTanksButtonWidget::DeferredShow()
{
...
void SBattleTanksButtonWidget::DeferredHide(bool bInstant)
{
	if (bInstant)
	{
...
bool SBattleTanksButtonWidget::IsAnimating() const
{
