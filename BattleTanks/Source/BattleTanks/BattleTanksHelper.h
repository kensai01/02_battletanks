...
#include "UI/BattleTanksMenuItem.h"
#include "UI/SBattleTanksMenuItem.h"

#pragma once

class FBattleTanksHelper
{
public:
...
namespace MenuHelper
{
	...
	FORCEINLINE FTimerHandle PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, UserClass* inObj, typename FBattleTanksMenuItem::FOnConfirmMenuItem::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
	...
	template< class UserClass >
	FORCEINLINE FTimerHandle PlaySoundAndCallSP(UWorld* World, const FSlateSound& Sound, UserClass* inObj, typename FBattleTanksMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		FSlateApplication::Get().PlaySound(Sound);
	...
	template< class UserClass >
	FORCEINLINE FTimerHandle PlaySoundAndCallSP(UWorld* World, const FSlateSound& Sound, UserClass* inObj, typename FBattleTanksMenuItem::FOnConfirmMenuItem::TSPMethodDelegate_Const< UserClass >::FMethodPtr inMethod)
	{
		FSlateApplication::Get().PlaySound(Sound);
